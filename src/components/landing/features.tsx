        ...
        <div className="pt-16 pb-12 md:pt-52 md:pb-20">
          ...
          <div>
            ...
            <div className="flex flex-col max-w-xl mx-auto md:max-w-none md:flex-row md:space-x-8 lg:space-x-16 xl:space-x-20 space-y-8 space-y-reverse md:space-y-0">
              ...
              { /* Content */}
              <div className="order-1 md:w-7/12 lg:w-1/2 md:order-none max-md:text-center" data-aos="fade-down">
                { /* Content #1 */}
                <div>
                  <div className="inline-flex pb-3 font-medium text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-purple-200">The security first platform</div>
                </div>
                <h3 className="pb-3 text-transparent h3 bg-clip-text bg-gradient-to-r from-revocalize-200/60 via-revocalize-100 to-revocalize-200/60">Simplify your security with authentication services</h3>
                <p className="mb-8 text-lg text-revocalize-100/60">Define access roles for the end-users, and extend your authorization capabilities to implement dynamic access control.</p>
                <div className="max-w-xs mt-8 max-md:mx-auto space-y-2">
                  <button className={`flex items-center text-sm font-medium text-slate-50 rounded rounded-lg border bg-slate-800/25 w-full px-4 py-3 transition duration-150 ease-in-out hover:opacity-100 ${tab !== 1 ? 'border-slate-700 opacity-50' : 'border-purple-700 shadow shadow-purple-500/25'}`} onClick={() => setTab(1)}>
                    ...
                    <span className='font-system'>Simplify your security</span>
                  </button>
                  <button className={`flex items-center text-sm font-medium text-slate-50 rounded rounded-lg border bg-slate-800/25 w-full px-4 py-3 transition duration-150 ease-in-out hover:opacity-100 ${tab !== 2 ? 'border-slate-700 opacity-50' : 'border-purple-700 shadow shadow-purple-500/25'}`} onClick={() => setTab(2)}>
                    <svg className="mr-3 shrink-0 fill-slate-300" xmlns="http://www.w3.org/2000/svg" width="16" height="16">
                      <path d="M2 6H0V2a2 2 0 0 1 2-2h4v2H2v4ZM16 6h-2V2h-4V0h4a2 2 0 0 1 2 2v4ZM14 16h-4v-2h4v-4h2v4a2 2 0 0 1-2 2ZM6 16H2a2 2 0 0 1-2-2v-4h2v4h4v2Z" />
                    </svg>
                    <span className='font-system'>Customer identity</span>
                  </button>
                  <button className={`flex items-center text-sm font-medium text-slate-50 rounded rounded-lg border bg-slate-800/25 w-full px-4 py-3 transition duration-150 ease-in-out hover:opacity-100 ${tab !== 3 ? 'border-slate-700 opacity-50' : 'border-purple-700 shadow shadow-purple-500/25'}`} onClick={() => setTab(3)}>
                    <svg className="mr-3 shrink-0 fill-slate-300" xmlns="http://www.w3.org/2000/svg" width="16" height="16">
                      <path d="M14.3.3c.4-.4 1-.4 1.4 0 .4.4.4 1 0 1.4l-8 8c-.2.2-.4.3-.7.3-.3 0-.5-.1-.7-.3-.4-.4-.4-1 0-1.4l8-8ZM15 7c.6 0 1 .4 1 1 0 4.4-3.6 8-8 8s-8-3.6-8-8 3.6-8 8-8c.6 0 1 .4 1 1s-.4 1-1 1C4.7 2 2 4.7 2 8s2.7 6 6 6 6-2.7 6-6c0-.6.4-1 1-1Z" />
                    </svg>
                    <span className='font-system'>Adaptable authentication</span>
                  </button>
                </div>
              </div>
                ...
                <div className="relative py-24 -mt-12">
                  ...
                  <div className="flex items-center justify-center">
                    <div className="relative flex items-center justify-center w-48 h-48">
                      { /* Halo effect */}
                      <svg className="absolute inset-0 pointer-events-none left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 will-change-transform blur-md" width="480" height="480" viewBox="0 0 480 480" xmlns="http://www.w3.org/2000/svg">
                        <defs>
