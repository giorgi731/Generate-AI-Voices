...
export class DFT {
  ...
  transform(real: number[], imag: number[]): void {
    const n = real.length;
    if (n !== imag.length) {
   ...
   */
  inverseTransform(real: number[], imag: number[]): void {
   ...
   */
  transformRadix2(real: number[], imag: number[]): void {
    // Length variables
    const n = real.length;
    if (n !== imag.length)
      throw new Error('Mismatched lengths');
   ...
   */
  transformBluestein(real: number[], imag: number[]): void {
   ...
   */
  convolveReal(x: number[], y: number[], out: number[]): void {
    const n = x.length;
   ...
   */
  convolveComplex(xreal: number[], ximag: number[], yreal: number[], yimag: number[], outreal: number[], outimag: number[]): void {
  ...
  newArrayOfZeros(n: number): number[] {
    const result = [];
    for (let i = 0; i < n; i++) {
      result.push(0);
    }
