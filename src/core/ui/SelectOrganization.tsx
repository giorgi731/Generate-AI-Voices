...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      ...
      dark:hover:border-black-100 dark:hover:bg-black-300 dark:focus:ring-offset-black-400`,
      className
    )}
    {...props}
  >
    {props.asChild ? (children) : (
      <div className="flex w-full items-center justify-between rounded-lg dark:bg-accent-bg dark:hover:bg-[#2B233A] p-[12px]">
        {/* <ProfileAvatar text={props.value?.toString()} /> */}
        <div className='inline-block w-full ml-[5px] text-left'>{children}</div>{' '}
        <div className="flex h-[22px] w-[22px] justify-center">
          {' '}
          <BiChevronDown className="w-6 h-6 text-white" />
        </div>
      </div>
    )}
  </SelectPrimitive.Trigger>
  ...
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={classNames(
        `animate-in fade-in-250 relative z-50 w-[250px] overflow-hidden border border-transparent
          border-t-gray-50 bg-white shadow-xl dark:border-[#251b38] 
          dark:bg-[#150e23] dark:shadow-[0_0_40px_0] dark:shadow-primary-600/10 rounded-lg`,
        className
      )}
