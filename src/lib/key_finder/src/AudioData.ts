export class AudioData {
  ...
  setSamplesFromAudioBuffer(audioBuffer: AudioBuffer): void {
    // Assuming the audio is mono. If it's stereo or more, you'll need to handle multiple channels.
    const channelData = audioBuffer.getChannelData(0);
    this.samples = Array.from(channelData);
    this.setChannels(audioBuffer.numberOfChannels);
    this.setFrameRate(audioBuffer.sampleRate);
  }
